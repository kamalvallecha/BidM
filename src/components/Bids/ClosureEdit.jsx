...
function ClosureEdit() {
        ...
        try {
          ...
          processedAudienceData.forEach(audience => {
            ...
            audience.countries.forEach(country => {
              newFormData[`${audience.id}_${selectedPartner}_${selectedLOI}_${country.name}`] = {
  ...
  const fetchBidDetails = async () => {
    try {
      setLoading(true);
      const data = await axios.get(`/bids/closure/${bidId}`);
      ...
      // Process audiences data
      Object.entries(data.audiences || {}).forEach(([audienceId, audience]) => {
        // Process metrics for each partner-LOI combination
        Object.entries(audience.metrics || {}).forEach(([partnerLoi, metrics]) => {
          const [partner, loi] = partnerLoi.split('_');
          
          // Set metrics
          const metricsKey = `metrics_${audienceId}_${partner}_${loi}`;
          newFormData[metricsKey] = {
            finalLOI: metrics.finalLOI?.toString() || '',
            finalIR: metrics.finalIR?.toString() || '',
            finalTimeline: metrics.finalTimeline?.toString() || '',
            qualityRejects: metrics.qualityRejects?.toString() || '',
            communication: metrics.communication?.toString() || '',
            engagement: metrics.engagement?.toString() || '',
            problemSolving: metrics.problemSolving?.toString() || '',
            additionalFeedback: metrics.additionalFeedback || ''
          };

          // Set field close date
          const fieldCloseDateKey = `${audienceId}_${partner}_${loi}_field_close_date`;
          newFormData[fieldCloseDateKey] = metrics.field_close_date || null;
        });

        // Process countries data
        Object.entries(audience.countries || {}).forEach(([country, countryData]) => {
          Object.entries(data.partners).forEach(([partner, partnerData]) => {
            partnerData.lois.forEach(loi => {
              const countryKey = `${audienceId}_${partner}_${loi}_${country}`;
              newFormData[countryKey] = {
                delivered: countryData.delivered?.toString() || ''
              };
            });
          });
        });
      ...
      // After processing backend data, check session storage
      const sessionData = sessionStorage.getItem(`bidClosure_${bidId}`);
      let finalFormData = { ...newFormData };
      
      if (sessionData) {
        const existingFormData = JSON.parse(sessionData);
        // Only merge session data if it has actual values
      ...
      console.log('Final form data after processing:', finalFormData); // Debug log
      ...
      // Convert audiences object to array and set it
      const audiencesArray = Object.entries(data.audiences || {}).map(([id, audience]) => ({
        id,
        name: audience.name,
        category: audience.category,
        countries: Object.entries(audience.countries || {}).map(([name, countryData]) => ({
          name,
          required: countryData.required || 0,
          allocation: countryData.allocation || 0,
          delivered: countryData.delivered || 0
        }))
      }));
      setAudienceData(audiencesArray);
      
      // Set initial partner and LOI if not already set
      if (!selectedPartner || !selectedLOI) {
        ...
        if (partners.length > 0) {
          ...
          if (data.partners[firstPartner] && Array.isArray(data.partners[firstPartner].lois)) {
            const lois = data.partners[firstPartner].lois;
            if (lois.length > 0) {
              setSelectedLOI(lois[0].toString());
            }
          }
        }
      }
  ...
  const handleDeliveredChange = (audienceId, country, value) => {
    setFormData(prev => ({
      ...prev,
      [`${audienceId}_${selectedPartner}_${selectedLOI}_${country}`]: {
        delivered: value
      }
    }));
  };
  ...
  const handleSave = async (submitAfterSave = false) => {
    try {
      ...
      const response = await axios.put(`/bids/${bidId}/closure`, {
        ...
        audienceData: audienceData.map(audience => {
          ...
          // Get n_delivered values for each country using the correct key format
          const countries = audience.countries.map(country => ({
            name: country.name,
            delivered: Number(formData[`${audience.id}_${selectedPartner}_${selectedLOI}_${country.name}`]?.delivered || 0)
          }));
  ...
  return (
    <div className="closure-edit-container">
      ...
      <Box sx={{ mt: 2, mb: 2 }}>
        ...
        <ToggleButtonGroup
          value={selectedLOI?.toString()}
          exclusive
          onChange={(e, newLOI) => {
            if (newLOI) setSelectedLOI(newLOI.toString());
          }}
        >
          {bidData?.partners[selectedPartner]?.lois.map(loi => (
            <ToggleButton key={loi} value={loi.toString()}>
              {loi} MIN
            </ToggleButton>
          ))}
        </ToggleButtonGroup>
                    ...
                    <TableCell>{country.allocation}</TableCell>
                        <TableCell>
                          <TextField
                            size="small"
                            type="number"
                            value={formData[`${audience.id}_${selectedPartner}_${selectedLOI}_${country.name}`]?.delivered ?? ''}
