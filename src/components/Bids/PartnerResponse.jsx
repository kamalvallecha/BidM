...
function PartnerResponse() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Check if this is a temp bid
        if (bidId.startsWith('temp_')) {
          const tempData = sessionStorage.getItem('basicDetailsFormData');
          console.log('Session storage data:', tempData);
          
          if (!tempData) {
            throw new Error('No bid data found');
          }
          formData = JSON.parse(tempData);
          console.log('Using session storage data:', formData);
          
          // If we have an actual ID in the session storage, use that
          if (formData.id) {
            actualBidId = formData.id;
          }
        }

        // Try to get from API using actual bid ID
        try {
          console.log('Fetching from API for bid:', actualBidId);
          formData = await callApi(`/bids/${actualBidId}`);
          console.log('API response:', formData);
          
          // Merge with session storage data to preserve any new partners/LOIs
          const savedData = sessionStorage.getItem('basicDetailsFormData');
          if (savedData) {
            const parsedSavedData = JSON.parse(savedData);
            formData = {
              ...formData,
              partners: [...new Set([...formData.partners, ...parsedSavedData.partners])],
              loi: [...new Set([...formData.loi, ...parsedSavedData.loi])]
            };
          }
        } catch (error) {
          // If API fails, try session storage as fallback
          const tempData = sessionStorage.getItem('basicDetailsFormData');
          if (tempData) {
            formData = JSON.parse(tempData);
            console.log('Falling back to session storage:', formData);
          } else {
            throw error;
          }
        }
        ...
        // Load partners
        const allPartners = await callApi('/partners');
        ...
        if (!bidId.startsWith('temp_')) {
          ...
          const responseData = await callApi(`/bids/${actualBidId}/responses`);
          setResponses(responseData.responses || {});
          setPartnerSettings(responseData.settings || {});
        } else {
          ...
          // Save initial responses to session storage
  ...
  useEffect(() => {
    if (bidData?.loi?.length > 0 && !selectedLOI) {
      setSelectedLOI(bidData.loi[0].toString());
    }
  }, [bidData]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Send all responses to backend using the actual bid ID from bidData
      const response = await callApi(`/bids/${bidId}/partner-responses`, {
        method: 'PUT',
        body: JSON.stringify({
